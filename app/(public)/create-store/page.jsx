...
export default function CreateStore() {
  ...
  const fetchSellerStatus = async () => {
    ...
    try {
      ...
      const currentStatus = data?.store?.status;

      if (["approved", "rejected", "pending"].includes(currentStatus)) {
        setStatus(currentStatus);
        setAlreadySubmitted(true);
        switch (currentStatus) {
          case "approved":
            setMessage(
              "Your store has been approved, you can now add products to your store from your dashboard",
            );
          ...
          case "pending":
            setMessage(
              "Your store request is pending, please wait for admin approval",
            );
            break;
